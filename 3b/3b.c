...
int load2(Table* t, char* fn){// робит 
    ...
    fread(t->ks, sizeof(KeySpace), t->csize, t->fd);// считываем таблицу
    return 1;
}
...
int main(){
    const char* choice[]={"0- Exit", "1- Add element", "2- Find element",
                          ...
                          "5- Add elements from a text file", "6- Reorganize the table"};
    const int N=sizeof(choice)/sizeof(choice[0]);
    int (*fptr[])(Table *) = {NULL, D_Add, D_Find, D_Del, D_Show, D_Load};
    Table t = {NULL, 0, 0, NULL};
    int rc;
    char* fn;
    if (D_Load2(&t, &fn) == 0)return 1;
    while(rc = dialog(choice, N)){
    	if(rc==6) D_Reorg(&t, fn);
    	if(rc!=6 && !fptr[rc](&t))break;
    	
    }
    D_Save(&t, &fn); // Выгружаем таблицу, закрываем файл данных 
