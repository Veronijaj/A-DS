...
int timing(Tree* t){
	...
	while(k!=1000000){
		...
		printf("??%ld??",time_total);
		fprintf(file,"%ld\n",time_total);
		time_total=0;
	//	destr(t);
		i++;
	//	fwrite(&lt[1],sizeof(char),1,file);
	//	k++;
	}
	fclose(file);
	return 0;
...
int D_Timing(Tree* t){
	Node *root = t->root;
	//int=51;
	int n = 10, k, cnt = 1000000, i, m;
	char* key[10000];
	clock_t first, last;
	srand(time(NULL));
	while (n-- > 0){
	//	for (i = 0; i < 10000; ++i)key[i] = (rand()%26+'A') * (rand()%26+'A');
		for (i = 0; i < cnt; ){
			char* key = rand_str();
			add_el(t, key,51);
			++i;
...
int main(){
    ...
    "3- Delete element", "4- Show table", "5- Special find", "6- Add elements from a text file", "7- Draw a tree", "8- Timing"};
    const int N=sizeof(choice)/sizeof(choice[0]);
    int (*fptr[])(Tree*) = {NULL, D_Add, D_Find, D_Del, D_Show, D_Special, D_Load, D_Draw, D_Timing};
