...
int print(Graph* g){
    ...
    for(int i=0; i<g->csize; i++){
        ...
        while(ptr!=NULL){
            printf("%s(%d)",g->data[ptr->adjacency]->name, g->data[ptr->adjacency]->port);
...
//	g1->msize=g->msize;
	...
	for(int k=0; k<i; k++){
		int p=eg[k]->v1;
		int q=eg[k]->v2;
		if(g1->data[p]->color!=1 || g1->data[q]->color!=1){
			add_edge(g1, g1->data[p]->name, g1->data[q]->name, eg[k]->weight->delay);
			g1->data[p]->color=1;
			g1->data[q]->color=1;
		}
	}
...
int comparator(const void* p, const void* q){
	int k=((Edges*)p)->weight->delay - ((Edges*)q)->weight->delay;
	if(k<0)return -1;
...
int main(){
    const char* choice[]={"0- Exit", "1- Add computer", "2- Add network",
    "3- Delete computer", "4- Delete network", "5- Show graph adjacency list", 
    "6- Change current computer", "7- Change current network", "8- Treverse",
    "9- Finding the shortest path", "10- Ostov"};
    const int N=sizeof(choice)/sizeof(choice[0]);
    int (*fptr[])(Graph*) = {NULL, D_Add_Vert, D_Add_Edge, D_Del_Vert, D_Del_Edge, D_Show, D_Change_Vertex, D_Change_Edge, D_Treverse, D_Find_Path, D_Mini};
